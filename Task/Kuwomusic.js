...
注意⚠️：当前脚本只测试Loon&Quantumult X，其他自测！
...
const kuwourlArr = [], kuwohdArr = [], kuwobodyArr = [], kuwocount = ''
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        kuwourlArr.push($.getdata('kuwourl'))
        kuwohdArr.push($.getdata('kuwohd'))
        kuwobodyArr.push($.getdata('kuwobody'))

        let kuwocount = ($.getval('kuwocount') || '1');

        for (let i = 2; i <= kuwocount; i++) {
        ...
        for (let i = 0; i < kuwohdArr.length; i++) {
