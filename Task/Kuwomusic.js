...
function kuwock() {
    if ($request.url.indexOf("freemium/h5/switches") > -1) {
        ...
        const index = status ? status : 1; // 如果 status 未定义，则默认为 1

        if (kuwourl) {
            $.setdata(kuwourl, `kuwourl${index}`);
            $.log(`保存的 kuwourl${index}: ${kuwourl}`);
        }

        const kuwohd = JSON.stringify($request.headers);
        if (kuwohd) {
            $.setdata(kuwohd, `kuwohd${index}`);
            $.log(`保存的 kuwohd${index}: ${kuwohd}`);
        }

        const kuwobody = $request.body;
        if (kuwobody) {
            $.setdata(kuwobody, `kuwobody${index}`);
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if (data.code == 200) {
                    ...
                    console.log('酷我音乐看广告 : ' + data.msg + '！等明天吧！❎');
                    message += '酷我音乐看广告 : ' + data.msg + '！等明天吧！❎\n';
                } else {
